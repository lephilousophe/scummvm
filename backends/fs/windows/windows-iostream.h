 ...
 */
class WindowsIoStream final : public StdioStream {
public:
	static StdioStream *makeFromPath(const Common::String &path, StdioStream::WriteMode writeMode) {
		return StdioStream::makeFromPathHelper(path, writeMode, [](void *handle) -> StdioStream * {
			return new WindowsIoStream(handle);
		});
	}
	WindowsIoStream(void *handle);

protected:
	bool moveFile(const Common::String &src, const Common::String &dst) override;
};
